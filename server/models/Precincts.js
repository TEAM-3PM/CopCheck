...
class Precincts {
	...
	static async findByName(name) {
		const query = `SELECT * FROM precincts WHERE LOWER(name) LIKE ?`;
		const result = await knex.raw(query, [`%${name.toLowerCase()}%`]);
		return result.rows;
	}

	static async findByAddress(address) {
		const query = `SELECT * FROM precincts WHERE LOWER(address) LIKE ?`;
		const result = await knex.raw(query, [`%${address.toLowerCase()}%`]);
		return result.rows;
	}
