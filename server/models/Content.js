...
class Content {
	...
	static async create(report_id, content, type, user_id) {
		const query = `INSERT INTO contents (report_id, content, type, user_id)
      VALUES (?, ?, ?, ?) RETURNING *`;
		const result = await knex.raw(query, [report_id, content, type, user_id]);
		const rawUserData = result.rows[0];
		return rawUserData;
	}
